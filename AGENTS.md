...
Phase 3 has started as a non-persistent, static/local, synthetic-only Case Review Command Center shell at `/case-command-center`. Phase 3.6 adds customer-safe wording module polish for that local shell: static suggested response variants, wording intent/tone labels, support-safe guardrails, selected-evidence rationale links, rep checklist, and not-sent/not-saved boundary copy. The route remains unlinked, off-white/parchment, and local-only; `/` remains the Phase 1 receipt workflow and does not link to `/case-command-center`; `ClaimReviewWorkflow` remains unchanged; `ProductPhotoReviewPanel` remains unrouted; no editable saved text, send/copy action, form submission, persistence, upload behavior, integrations, OCR/metadata, storage/providers, auth/billing, real evidence, receipt behavior changes, deployment, or Phase 3.7 implementation has been added.
...
- Phase 3: Case Review Workflow. Started as a static/local, synthetic-only, unlinked `/case-command-center` shell; Phase 3.6 adds customer-safe wording module polish only, with no live workflow, persistence, send/copy action, or integration behavior.
