...
- Phase 4: Stronger OCR/AI service integrations. Phase 4.0 planning-only real AI/OCR/photo intelligence readiness is documented; next safe milestone is Phase 4.1 OCR/provider architecture planning. Do not implement real AI, OCR, photo analysis, providers/SDKs, credentials, env vars, persistence, routes, upload behavior, real evidence handling, `ClaimReviewWorkflow` wiring, `ProductPhotoReviewPanel` routing, `LocalAnalysisResult` migration, or receipt behavior changes until Robert explicitly opens a named implementation slice.
