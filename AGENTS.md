...
Phase 4.0 planning-only real AI/OCR/photo intelligence readiness is documented in `PHASE_4_0_AI_OCR_PHOTO_INTELLIGENCE_READINESS.md`. Phase 4.1 docs-only OCR/provider architecture planning is documented in `PHASE_4_1_OCR_PROVIDER_ARCHITECTURE_PLAN.md`. Phase 4 implementation has not started. Stronger OCR, AI/vision, and photo intelligence should be planned as evidence review signals with confidence, uncertainty, limitations, and manual-review guidance, not final verdicts, fraud conclusions, or automated support decisions. Phase 4.1 defines provider-neutral OCR input/output boundaries, text blocks, structured receipt fields, Amazon validation readiness, browser-local/server-side tradeoffs, future route boundaries, privacy/data handling, scoring/signal integration, QA/safety gates, and blocked scope without adding providers, routes, SDKs, env vars, upload behavior, storage, persistence, real evidence handling, `ClaimReviewWorkflow` wiring, `ProductPhotoReviewPanel` routing, `LocalAnalysisResult` migration, receipt behavior changes, deployment, commit, or push.
...
- Phase 4: Stronger OCR/AI service integrations. Phase 4.0 planning-only real AI/OCR/photo intelligence readiness and Phase 4.1 docs-only OCR/provider architecture planning are documented; next safe milestone is Phase 4.2 synthetic OCR fixture harness work. Do not implement real AI, OCR provider calls, photo analysis, providers/SDKs, credentials, env vars, persistence, routes, upload behavior, real evidence handling, `ClaimReviewWorkflow` wiring, `ProductPhotoReviewPanel` routing, `LocalAnalysisResult` migration, or receipt behavior changes until Robert explicitly opens a named implementation slice.
