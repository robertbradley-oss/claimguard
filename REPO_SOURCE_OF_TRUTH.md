...
- Phase 2.9 is planning-only next. Do not start Phase 2.9 implementation, wire the wrapper into live callers, or change runtime/UI/upload/report behavior without explicit approval.
- Phase 2.9.0 is docs-only planning in `PHASE_2_9_0_UNSUPPORTED_EVIDENCE_LIVE_OPT_IN_READINESS_PLAN.md`. It decides unsupported evidence is not ready for live opt-in implementation yet. The next safe move is review/commit/push of that plan, followed by a review-only Phase 2.9.1 / Phase 2 closeout. Phase 2 can close as a non-live readiness phase only if that closeout finds no blocker. Do not start Phase 3 implementation from this checkpoint.
