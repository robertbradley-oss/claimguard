...
- Phase 4.3 non-live provider-neutral OCR extraction contract is implemented in `src/lib/analysis/ocr-extraction-contract.ts` with probe coverage in `src/lib/analysis/ocr-extraction-contract.probe.ts`, registered in `scripts/run-product-photo-probes.cjs`, and covered by `scripts/check-report-semantics.mjs`. It normalizes Phase 4.2 synthetic fixture outputs into provider-neutral receipt extraction signals with field confidence, extraction confidence, manual-review drivers, limitations, unsupported outcomes, synthetic provider-failure outcomes, safe summaries, review signal levels, and manual-review requirements. It remains synthetic-only, provider-free, route-free, UI-free, upload-free, storage-free, real-evidence-free, and unwired from live runtime.
- Phase 4.4 planning-only server-side OCR route and data-flow design is documented in `PHASE_4_4_SERVER_SIDE_OCR_ROUTE_DATA_FLOW_PLAN.md`. It plans a future `/api/analysis/ocr` route boundary, temporary evidence flow, no-retention policy, provider payload minimization, timeout/failure behavior, cost/abuse controls, security prerequisites, Phase 4.3 contract integration, safety language, and Phase 4.5 gates. It does not implement a route, add providers/SDKs/env vars/uploads/storage/persistence, process real evidence, wire live workflow, migrate `LocalAnalysisResult`, or change receipt behavior.
- The next safe task is Phase 4.5 server-side OCR route skeleton implementation planning only, unless Robert explicitly approves a synthetic-only/mock-provider route skeleton. Do not implement real AI, OCR provider calls, provider SDKs, credentials, env vars, live routes, upload behavior, persistence, live scoring, real evidence handling, `ClaimReviewWorkflow` wiring, `ProductPhotoReviewPanel` routing, `LocalAnalysisResult` migration, or receipt behavior changes until Robert explicitly opens a named implementation slice.
