...
```text
/claimguardagent implement Phase 4.27 synthetic fixture metadata only: add package-safe synthetic OpenAI Vision sandbox metadata entries with required safety fields, no images/files of evidence, no runtime schema/types, no route behavior changes, no SDKs/env vars/provider calls, no uploads/storage/persistence, no real/anonymized customer evidence, no public/object URLs or storage handles, no provider payloads/raw OCR dumps, no package artifacts, no UI, no receipt behavior changes, no `analyzeEvidenceFile` or `LocalAnalysisResult` changes, and run `check:vision-sandbox-boundaries` with the existing gates
```
