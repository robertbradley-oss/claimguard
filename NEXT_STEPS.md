...
```text
/claimguardagent review and, if approved, commit the docs-only Phase 2.8.1 unsupported-evidence dev bridge implementation plan if docs-safe checks pass; do not push, deploy, implement the dev route, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, or start Phase 2.9
```
