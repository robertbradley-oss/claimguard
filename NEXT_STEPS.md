...
1. Review and, if approved, commit the docs-only Phase 2.9.0 unsupported-evidence live-opt-in readiness plan after docs-safe checks pass. Do not implement runtime behavior, wire live callers, add unsupported-evidence UI to the main workflow, edit upload/live report adapter mapping, migrate `LocalAnalysisResult`, change receipt behavior, route to `ProductPhotoReviewPanel`, add providers/storage/integrations/case queues, add real photos/metadata fixtures, deploy, or push without explicit approval.
2. After the Phase 2.9.0 plan is pushed, run a review-only Phase 2.9.1 / Phase 2 closeout to decide whether Phase 2 can close as a non-live readiness phase.
...
11. Do not start Phase 2.9 implementation; Phase 2.9 is planning-only until Robert explicitly opens a runtime/UI slice.
...
```text
/claimguardagent review and, if approved, commit the docs-only Phase 2.9.0 unsupported-evidence live-opt-in readiness plan if checks pass; do not push, deploy, implement runtime, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, or start Phase 3 implementation
```
