...
```text
/claimguardagent prepare Phase 4.18 mock provider route developer documentation and usage examples only: document safe synthetic JSON calls to POST /api/analysis/mock-provider for mock OCR, mock vision, failure modes, validation failures, privacy flags, and route/contract separation; do not change route behavior, do not add providers/SDKs/env vars/uploads/storage/persistence/UI/live analyzer wiring, do not process real evidence, and do not start live AI/Vision/photo analysis
```
