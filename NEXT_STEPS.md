...
```text
/claimguardagent implement Phase 3.3 visual hierarchy and selected-evidence structure polish for /case-command-center only; keep it mock/local and synthetic-only; do not add persistence, upload behavior, integrations, OCR/metadata, storage/providers, auth/billing, real evidence, unsupported-evidence runtime wiring, ProductPhotoReviewPanel routing, ClaimReviewWorkflow edits, analyzeEvidenceFile edits, LocalAnalysisResult migration, report/parser/scoring/fixture changes, deployment, or push
```
