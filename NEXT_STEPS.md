...
1. Review and, if approved, commit the docs-only Phase 2.7.1 unsupported-evidence reviewer state contract. Do not start Phase 2.7 implementation, wire the wrapper into live callers, add unsupported-evidence UI, edit upload/UI/live report adapter mapping, migrate `LocalAnalysisResult`, add product-photo analyzer/report behavior, route to `ProductPhotoReviewPanel`, add providers/storage/integrations/case queues, add real photos/metadata fixtures, deploy, or push without explicit approval.
...
```text
/claimguardagent commit the docs-only Phase 2.7.1 unsupported-evidence reviewer state contract if the diff is still limited to documentation and docs-safe checks pass; do not push, deploy, edit runtime code, add unsupported-evidence UI, wire the wrapper into live callers, route ProductPhotoReviewPanel, or change receipt behavior
```
