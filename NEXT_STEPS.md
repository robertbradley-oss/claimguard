...
```text
/claimguardagent run Phase 4.8 as route hardening and developer-only fixture test documentation only: document accepted fixture-key examples, safe local smoke instructions, expected validation failures, route safety boundaries, and non-live limitations for POST /api/analysis/ocr; do not add providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR
```
