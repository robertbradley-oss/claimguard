...
```text
/claimguardagent run a review-only Phase 3 closeout readiness/source-of-truth checkpoint for /case-command-center; verify the Phase 3.10 final shell remains static/local, synthetic-only, unlinked, off-white ClaimGuard direction, non-scoring, decision-free, action-free, responsive on desktop/mobile, and unchanged for receipt behavior, ClaimReviewWorkflow, ProductPhotoReviewPanel, upload, file inputs, saved evidence state/form submission, persistence, integrations, OCR/metadata, storage/providers, auth/billing, real evidence, analyzer/report/parser/scoring/fixture behavior, deployment, and Phase 4
```
