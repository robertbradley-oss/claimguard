...
```text
/claimguardagent document OpenAI Vision sandbox developer usage and readiness checkpoint only: keep it docs-only with no SDKs, env vars, provider calls, new route, existing route behavior changes, upload/storage/persistence, UI wiring, fixture changes, package artifacts, real evidence, analyzeEvidenceFile changes, LocalAnalysisResult changes, receipt behavior changes, or deployment
```
