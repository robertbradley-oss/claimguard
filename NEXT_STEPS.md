...
```text
/claimguardagent run a review-only Phase 3.8 post-push QA/source-of-truth closeout for /case-command-center; verify the case header/command-status polish remains static/local, synthetic-only, unlinked, off-white ClaimGuard direction, non-clickable/action-free, and unchanged for receipt behavior, ClaimReviewWorkflow, ProductPhotoReviewPanel, upload, file inputs, saved evidence state/form submission, persistence, integrations, OCR/metadata, storage/providers, auth/billing, real evidence, analyzer/report/parser/scoring/fixture behavior, and deployment
```
