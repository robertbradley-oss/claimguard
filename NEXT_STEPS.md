...
```text
/claimguardagent run Phase 4.1 OCR/provider architecture planning only: define provider-neutral OCR boundaries, data-flow/privacy rules, receipt-regression gates, provider-failure behavior, cost/latency constraints, and implementation stop conditions; do not add providers, SDKs, credentials, env vars, routes, upload behavior, persistence, AI/vision calls, product-photo runtime, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, receipt behavior changes, deployment, commit, or push
```
