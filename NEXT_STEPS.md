...
1. Run Phase 2.5.2 only after Robert explicitly opens it: a no-live dev-only adapter-readiness review harness per `PHASE_2_5_DEV_HARNESS_SCOPE_PLAN.md` and `PRODUCT_PHOTO_DEV_HARNESS_PLAN.md` (literal synthetic `ProductPhotoAdapterReadinessResult` cases on a production-disabled, unlinked `/dev/product-photo-adapter-readiness` route with type-only imports), mirroring the Phase 2.5.1 gate harness pattern. Keep `prepareProductPhotoAdapterReadinessForDevOnlyBoundary` and `buildPreAnalysisEvidenceGateDecision` decision-only and unwired; do not touch upload, UI, live report mapping, the live receipt analyzer entrypoint, the receipt-shaped result, providers, storage, integrations, case queues, or runtime. Alternatively, run a review-only Phase 2.5 closeout.
