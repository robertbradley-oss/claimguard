...
- Do not implement live OCR/provider work before the Phase 4.3 provider-neutral OCR extraction contract is explicitly opened as non-live and unwired.
...
```text
/claimguardagent run Phase 4.3 provider-neutral OCR extraction contract implementation only: add non-live/unwired contract types or helpers that align with the Phase 4.1 architecture and Phase 4.2 synthetic fixture harness; do not call OCR providers, add SDKs, env vars, routes, upload behavior, storage, persistence, AI/Vision calls, real evidence, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, receipt behavior changes, deployment, commit, or push unless explicitly approved
```
