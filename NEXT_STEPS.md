...
```text
/claimguardagent plan Phase 4.15 as mock adapter route integration planning only: document whether and how a future synthetic route milestone could use the mock provider adapter without changing the current exact fixture-key POST /api/analysis/ocr behavior yet; do not implement route wiring, live providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR; run required documentation and safety checks; commit and push only if all gates pass
```
