...
- Do not add provider SDKs, credentials, env vars, server analysis routes, persistence, storage, or real evidence handling.
...
```text
/claimguardagent run Phase 4.5 server-side OCR route skeleton implementation plan only: define a synthetic-only, production-disabled/mock-provider route skeleton scope, request validation probes, safe failure states, privacy scans, and receipt-preservation gates without implementing routes, providers, SDKs, env vars, uploads, storage, persistence, real evidence processing, AI/Vision calls, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, receipt behavior changes, deployment, commit, or push unless explicitly approved
```
