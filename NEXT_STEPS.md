...
- Phase 3.1 is complete as a planning/design-only Case Review Command Center concept in `PHASE_3_1_CASE_WORKFLOW_DESIGN_CONCEPT.md`. It defines dark forensic enterprise visual direction, case workflow layout, planned design-system primitives, interaction model, safety/wording rules, technical boundaries, and the recommended Phase 3.2 first-build target without implementing routes, components, runtime wiring, live unsupported-evidence UI, ProductPhotoReviewPanel routing, persistence, integrations, OCR/metadata/image analysis, `analyzeEvidenceFile` changes, `LocalAnalysisResult` migration, receipt behavior changes, deployment, commit, or push.
- Phase 3.2 is implemented locally as a non-persistent Case Review Command Center shell at `/case-command-center`. It adds an unlinked route, `src/components/CaseReviewCommandCenter.tsx`, and isolated synthetic mock case data in `src/lib/case-command-center/mock-case.ts`. The shell provides a case header/status strip, evidence sidebar, selected evidence panel, case-level review summary, notes/manual decision placeholder, customer-safe wording panel, and timeline placeholder. It uses mock/local data only and does not edit `ClaimReviewWorkflow`, wire unsupported-evidence runtime, route `ProductPhotoReviewPanel`, change `analyzeEvidenceFile`, migrate `LocalAnalysisResult`, change receipt upload/analyzer/report/parser/scoring/provider/storage behavior, add persistence, integrations, OCR/metadata/image analysis, deployment, commit, or push.
...
```text
/claimguardagent review and, if approved, commit Phase 3.2 non-persistent local Case Review Command Center shell; verify /case-command-center, lint, build, report semantics, product-photo probes, diff check, and protected receipt/analyzer scans; do not deploy or push
```
