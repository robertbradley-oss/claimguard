...
- Do not implement OCR/provider work before the Phase 4.2 synthetic OCR fixture harness proves fixture-only expectations and receipt-regression boundaries.
...
```text
/claimguardagent run Phase 4.2 synthetic OCR fixture harness planning/implementation only: define or add synthetic OCR fixtures and expected normalized OCR/structured receipt outputs with no real evidence and no provider calls; keep analyzeEvidenceFile, LocalAnalysisResult, upload behavior, receipt report mapping, ClaimReviewWorkflow, ProductPhotoReviewPanel, providers, SDKs, env vars, routes, storage, persistence, AI/Vision calls, live scoring, deployment, commit, and push out of scope unless explicitly approved
```
