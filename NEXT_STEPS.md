...
```text
/claimguardagent implement Phase 4.26 local static OpenAI Vision sandbox validation/probe checks only: add no SDK/env/provider-call, no upload/storage/object URL, protected runtime/route boundary, privacy/identifier, provider payload/raw OCR, unsafe wording, altered-or-AI-generated-image uncertainty, observation-vs-signal, fixture metadata policy, and package-safety scans; no dependencies, no SDKs/env vars/provider calls, no route behavior changes, no runtime schema/types, no fixture files/images, no UI, no real evidence, no package artifacts, no receipt behavior changes, no `analyzeEvidenceFile` or `LocalAnalysisResult` changes, and no live AI/Vision/photo analysis
```
