...
1. Keep the `pre-analysis-evidence-gate` contract/probe (`src/lib/analysis/pre-analysis-evidence-gate.ts` / `.probe.ts`) decision-only and unwired. Do not wire `buildPreAnalysisEvidenceGateDecision` into upload, UI, live report mapping, the live receipt analyzer entrypoint, the receipt-shaped result, providers, storage, integrations, case queues, or runtime until Robert explicitly opens a runtime slice. The next safe milestone is the dev-only adapter review harness in `PRODUCT_PHOTO_DEV_HARNESS_PLAN.md` (still no-live), with optional `REPO_SOURCE_OF_TRUTH.md` cleanup if Robert requests a tiny docs alignment pass.
