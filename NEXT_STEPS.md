...
1. Review and, if approved, commit the docs-only Phase 2.8.0 unsupported-evidence dev bridge plan. Do not implement the dev route, wire live callers, add unsupported-evidence UI to the main workflow, edit upload/live report adapter mapping, migrate `LocalAnalysisResult`, change receipt behavior, route to `ProductPhotoReviewPanel`, add providers/storage/integrations/case queues, add real photos/metadata fixtures, start Phase 2.9, deploy, or push without explicit approval.
...
10. Do not start Phase 2.9 until Phase 2.8 defines or proves the dev-only/user-visible bridge from the unsupported-evidence boundary to a safe reviewer surface.
...
```text
/claimguardagent review and, if approved, commit the docs-only Phase 2.8.0 unsupported-evidence dev bridge plan if docs-safe checks pass; do not push, deploy, implement the dev route, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, or start Phase 2.9
```
