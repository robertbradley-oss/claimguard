...
```text
/claimguardagent implement Phase 4.14 as mock adapter developer documentation and usage examples only: document how to call runMockOcrProvider and runMockVisionProvider with synthetic mock inputs, include safe expected-output examples for mock OCR success, mock vision success, timeout, unsupported, and privacy flags, keep examples synthetic-only and provider-free, do not wire the adapter to POST /api/analysis/ocr, do not add live providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR; update semantic coverage if needed; run lint, build, report-semantics, product-photo-probes, diff check, and targeted scans; commit and push only if all gates pass
```
