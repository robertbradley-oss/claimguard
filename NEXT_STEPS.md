...
1. Run Phase 2.4.10 as a no-live `pre-analysis-evidence-gate` contract/probe implementation only after Robert explicitly opens that slice. Limit it to a decision-only pre-OCR/pre-metadata gate, active probe registration, semantic/import/privacy guards, and status docs; do not wire it into upload, UI, live report mapping, `analyzeEvidenceFile`, `LocalAnalysisResult`, provider, storage, integration, case queue, real photo, real metadata fixture, deployment, or runtime work.
