...
- Existing `POST /api/analysis/ocr` remains exact `fixtureKey` only. Existing `POST /api/analysis/mock-provider` remains synthetic/mock-only and adapter-only. `analyzeEvidenceFile`, `LocalAnalysisResult`, receipt parser/scoring/report behavior, `ClaimReviewWorkflow`, and `ProductPhotoReviewPanel` remain unchanged.
- Phase 4.27 synthetic fixture metadata implementation decision:
- Phase 4.27 adds `fixtures/vision-sandbox/metadata/synthetic-fixture-registry.json` with package-safe synthetic metadata entries for clean receipt, suspicious-layout receipt, partial/cropped receipt, order screenshot missing context, product photo normal context, damaged product ambiguous context, altered-or-AI-generated-image uncertainty low/medium/high review concern, unsupported evidence, provider timeout simulation, and schema-validation-failure simulation.
- `PHASE_4_27_SYNTHETIC_FIXTURE_METADATA.md` documents metadata-only scope, required fields, safety/package posture, altered/AI uncertainty framing, validation coverage, specialist findings, and the Phase 4.28 recommendation.
- Existing `POST /api/analysis/ocr` remains exact `fixtureKey` only. Existing `POST /api/analysis/mock-provider` remains synthetic/mock-only and adapter-only. `analyzeEvidenceFile`, `LocalAnalysisResult`, receipt parser/scoring/report behavior, `ClaimReviewWorkflow`, and `ProductPhotoReviewPanel` remain unchanged.
- Phase 4.28 synthetic fixture creation planning decision:
...
- Existing `POST /api/analysis/ocr` remains exact `fixtureKey` only. Existing `POST /api/analysis/mock-provider` remains synthetic/mock-only and adapter-only. `analyzeEvidenceFile`, `LocalAnalysisResult`, receipt parser/scoring/report behavior, `ClaimReviewWorkflow`, and `ProductPhotoReviewPanel` remain unchanged.
- Phase 4.30 OpenAI Vision sandbox skeleton planning decision:
- Phase 4.30 plans the future developer-only sandbox skeleton without implementing it. `PHASE_4_30_OPENAI_VISION_SANDBOX_SKELETON_PLAN.md` defines future skeleton purpose, module boundaries, route boundary options, input/output boundaries, synthetic fixture and metadata relationships, validation/probe requirements, existing OCR/mock-provider route boundaries, package/downloadable safety, Phase 4.31 recommendation, and stop conditions.
- The safest Phase 4.31 path is script/module-only skeleton implementation first, with no API route unless separately approved. Future skeleton output must remain sandbox-only and separate from `LocalAnalysisResult`, live receipt reports, receipt scoring, claim decisions, `ClaimReviewWorkflow`, `ProductPhotoReviewPanel`, upload behavior, and production UI.
- Phase 4.30 adds no sandbox skeleton code, new route, route behavior change, runtime schema/types, fixture change, OpenAI/provider SDK, env var, provider call, upload/storage/persistence, UI, package artifact, deployment, live AI/Vision/photo analysis, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
...
- Phase 4.33 OpenAI Vision sandbox developer usage and readiness checkpoint decision:
- Phase 4.33 documents safe developer usage for the current script/module-only sandbox skeleton and fixture runner in `PHASE_4_33_VISION_SANDBOX_DEVELOPER_USAGE_READINESS.md`. It explains what the sandbox can and cannot do, safe npm commands, fixture/metadata rules, safety wording, protected route/runtime/receipt boundaries, package/downloadable safety, readiness for provider-configuration planning only, and stop conditions before API-credit-using work.
- Phase 4.33 remains documentation/readiness only. It adds no provider configuration, OpenAI/provider SDK, env var, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence/customer-data path, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- Phase 4.34 OpenAI Vision provider configuration planning decision:
- Phase 4.34 documents future provider configuration goals, explicit provider opt-in, disabled-by-default rules, API-credit usage boundaries, future `.env.example` guidance only, model/provider selection strategy, timeout/retry/rate/cost defaults, logging/payload-retention/privacy defaults, current sandbox relationship, downloadable/self-hosted package safety, approval gates, and Phase 4.35 safe options in `PHASE_4_34_OPENAI_VISION_PROVIDER_CONFIG_PLAN.md`.
- Phase 4.34 remains documentation/planning only. It adds no provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence/customer-data path, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- Phase 4.35 real OpenAI Vision sandbox implementation planning decision:
- Phase 4.35 documents the exact future implementation objective, future file-change boundaries, SDK versus minimal fetch adapter tradeoffs, future provider request and response flow, synthetic-only first-call rules, API-credit approval policy, future env/config requirements, prompt/schema/fixture-runner integration, downloadable/self-hosted package safety, future live-sandbox checks, Phase 4.36 recommendation, and stop conditions for the first API-credit-using implementation in `PHASE_4_35_OPENAI_VISION_SANDBOX_IMPLEMENTATION_PLAN.md`.
- Phase 4.35 remains implementation planning only. It adds no live OpenAI implementation, provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence/customer-data path, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- Phase 4.36 real OpenAI Vision sandbox approval checkpoint decision:
- Phase 4.36 documents explicit Robert approval choices for provider configuration skeleton work, first API-credit-using synthetic fixture calls, pausing live provider work, and downloadable/self-hosted setup planning in `PHASE_4_36_OPENAI_VISION_SANDBOX_APPROVAL_CHECKPOINT.md`. It confirms only Choice B authorizes API-credit-using work, defines work allowed without API-credit approval, work blocked without API-credit approval, first-live-call rules, provider-configuration skeleton readiness, API-credit readiness, stop/rollback conditions, package/downloadable safety, and specialist findings.
- Phase 4.36 remains approval checkpoint documentation only. It adds no live OpenAI implementation, provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence/customer-data path, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- Phase 4.37 OpenAI Vision provider configuration skeleton decision:
- Phase 4.37 implements a sandbox-only inert provider configuration skeleton in `src/lib/analysis/vision-sandbox/provider-config.ts` and documents it in `PHASE_4_37_OPENAI_VISION_PROVIDER_CONFIG_SKELETON.md`. The config defaults keep provider disabled, provider calls blocked, request execution blocked, API-credit usage blocked, payload logging disabled, raw OCR retention disabled, evidence scope synthetic-fixture-only, and package mode downloadable-safe-disabled.
- Phase 4.37 adds no `.env.example`, `.env`, `.env.local`, live OpenAI implementation, OpenAI/provider SDK, real env value, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence/customer-data path, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- The recommended next safe task is Phase 4.38 provider configuration skeleton hardening and `.env.example`/package-safety review, still no provider calls and no API-credit usage. Do not start live OpenAI Vision implementation unless Robert explicitly approves Choice B/API-credit-using work.
