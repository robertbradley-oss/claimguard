...
- The next safe options are Phase 4.20 OpenAI Vision sandbox prompt/output contract planning, Phase 4.20 OpenAI Vision sandbox implementation planning with no SDK/env/provider code, or a separately approved synthetic/anonymized-fixture-only sandbox skeleton. Live OCR/provider work, real evidence handling, SDKs, env vars, uploads, storage, persistence, UI wiring, receipt behavior changes, and `LocalAnalysisResult` migration remain blocked unless Robert explicitly opens a named implementation slice.
